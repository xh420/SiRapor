...
class LoginSystem:
    ...
    def is_guru(self):
        return self.is_logged_in() and self.current_user["role"] in ["guru"]
...
class SiRapor:
    ...
    def show_login_screen(self):
        ...
        ttk.Label(login_frame, text="SISTEM SiRAPOR", font=('Verda', 20, 'bold')).grid(row=1, column=0, columnspan=2, pady=(0,10))
    ...
    def Applikasi_Utama(self):
        ...
        self.jembot = ttk.Frame(self.notebook)
    ...
    def edit_selected_student(self):
        ...
        student_name = self.search_tree.item(selected[0])['values'][0]
        student_class = self.search_tree.item(selected[0])['values'][1]
        
        student_data = None
        for siswa in self.data_siswa:
            if siswa['nama'] == student_name and siswa['kelas'] == student_class:
                student_data = siswa
                break
    ...
    def delete_selected_student(self):
        ...
        if messagebox.askyesno("Konfirmasi", f"Hapus data {student_name} ({student_class})?"):
            self.data_siswa = [s for s in self.data_siswa if not (s['nama'] == student_name and s['kelas'] == student_class)]
